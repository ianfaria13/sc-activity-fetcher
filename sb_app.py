...
def fetch_group_names(token, org_id):
    """抓所有 groups，建立 {groupId: groupName} 字典"""
    params = {"PageSize": 2000, "IncludeArchived": False}
    if org_id: params["OrganisationIds"] = org_id
    try:
        data   = api_get(token, "/api/v1/Groups/Group", params)
        groups = data.get("data") or []
        return {g["id"]: g.get("name") or g.get("shortName") or "" for g in groups}
    except Exception:
        return {}
...
if fetch_btn:
    ...
    with st.spinner("Loading activity names..."):
        group_names = fetch_group_names(token, org_id)
        ...
        if not events:
            ...
            for ev in events:
                ...
                rows.append({
                    ...
                    "Homeroom":        homeroom,
                    "Parent Email(s)": parent_str,
                    "Activity Name":   group_names.get(ev.get("groupId"), "(Unknown Group)"),
