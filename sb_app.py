"""
SchoolsBuddy Activity Fetcher
Run: python3 -m streamlit run sb_app.py
"""

import io
import time

import streamlit as st
import requests
import pandas as pd
from datetime import datetime, date, timezone, timedelta
from openpyxl.styles import Alignment, Font, PatternFill
from openpyxl.utils import get_column_letter

# ── CONFIG ───────────────────────────────────────────────────
DEFAULT_CLIENT_ID     = "ian"
DEFAULT_CLIENT_SECRET = ""
DEFAULT_ORG_ID        = "297"
AUTH_URL  = "https://accounts2.schoolsbuddy.net/connect/token"
BASE_URL  = "https://publicapi-asia.schoolsbuddy.net"
LOGO_B64  = ""
# ─────────────────────────────────────────────────────────────

st.set_page_config(
    page_title="SchoolsBuddy Activity Fetcher",
    page_icon="🎓",
    layout="wide",
    initial_sidebar_state="expanded",
)

st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
html, body, [class*="css"] { font-family: 'Inter', sans-serif !important; }

[data-testid="stAppViewContainer"] { background: #f5f3fb; }
[data-testid="stHeader"] { display: none !important; }
[data-testid="collapsedControl"] { display: none !important; }

[data-testid="stSidebar"] {
    background: #3b1a5e !important;
    min-width: 270px !important;
}
[data-testid="stSidebar"] * { color: rgba(255,255,255,0.9) !important; }
[data-testid="stSidebar"] input {
    background: white !important;
    border: 1px solid rgba(255,255,255,0.25) !important;
    border-radius: 8px !important;
    color: #1a1a1a !important;
}
[data-testid="stSidebar"] input::placeholder { color: #aaaaaa !important; }
[data-testid="stSidebar"] hr { border-color: rgba(255,255,255,0.15) !important; }

/* Date/number pickers and the password show/hide icon all live inside a
   BaseWeb "input" wrapper with a light background -- the wildcard rule
   above turns their text/icons white-on-white on some Streamlit versions
   (this only showed up on Streamlit Cloud, not locally, because the exact
   DOM BaseWeb renders for these controls differs by Streamlit version). Force
   everything inside that wrapper dark so it's always legible. */
[data-testid="stSidebar"] [data-baseweb="input"] * ,
[data-testid="stSidebar"] [data-baseweb="base-input"] * ,
[data-testid="stSidebar"] [data-testid="stDateInput"] [data-baseweb] * {
    color: #1a1a1a !important;
    fill: #1a1a1a !important;
}

/* The calendar pop-up for date_input renders on a white panel -- force dark
   text there too, and cover both the case where it's nested under the
   sidebar and the case where BaseWeb portals it up to the document root.
   Only the outer panel gets a forced white background -- forcing it on
   every descendant too would wipe out the selected-day highlight color. */
[data-baseweb="calendar"] { background: white !important; }
[data-baseweb="calendar"] * { color: #1a1a1a !important; }

.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #d4489a, #7c3abf) !important;
    color: white !important;
    border: none !important;
    border-radius: 8px !important;
    font-weight: 600 !important;
    font-size: 14px !important;
    padding: 0.55rem 1rem !important;
    width: 100% !important;
}
.stButton > button[kind="primary"]:hover { opacity: 0.87 !important; }
.stButton > button:not([kind="primary"]) {
    background: rgba(255,255,255,0.1) !important;
    color: white !important;
    border: 1px solid rgba(255,255,255,0.25) !important;
    border-radius: 8px !important;
    width: 100% !important;
}

[data-testid="metric-container"] {
    background: white !important;
    border: 1px solid #e2daf2 !important;
    border-radius: 12px !important;
    padding: 1rem 1.25rem !important;
}
[data-testid="stMetricLabel"] p { color: #8b75b8 !important; font-size: 12px !important; font-weight: 600 !important; text-transform: uppercase !important; letter-spacing: 0.06em !important; }
[data-testid="stMetricValue"] { color: #3b1a5e !important; font-weight: 700 !important; }

[data-testid="stDataFrame"] {
    border-radius: 12px !important;
    border: 1px solid #e2daf2 !important;
    overflow: hidden !important;
}

.stTextInput label { color: #3b1a5e !important; font-weight: 600 !important; font-size: 13px !important; }
.stTextInput input { border: 1.5px solid #d6cef0 !important; border-radius: 8px !important; }
.stTextInput input:focus { border-color: #7c3abf !important; box-shadow: 0 0 0 3px rgba(124,58,191,0.1) !important; }

[data-testid="stDownloadButton"] button {
    background: #3b1a5e !important;
    color: white !important;
    border: none !important;
    border-radius: 8px !important;
    font-weight: 600 !important;
}

.stProgress > div > div { background: linear-gradient(90deg, #7c3abf, #d4489a) !important; border-radius: 99px !important; }
hr { border-color: #e2daf2 !important; }
</style>
""", unsafe_allow_html=True)

# ── Sidebar ───────────────────────────────────────────────────
with st.sidebar:
    st.markdown(f"""
    <div style="padding: 1.25rem 0 1rem; border-bottom: 1px solid rgba(255,255,255,0.15); margin-bottom: 1rem; text-align: center;">
        <img src="data:image/png;base64,{LOGO_B64}" style="width: 64px; height: 64px; border-radius: 50%;" />
        <p style="font-size: 16px; font-weight: 700; color: white; margin: 8px 0 2px;">SchoolsBuddy</p>
        <p style="font-size: 11px; color: rgba(255,255,255,0.5); margin: 0; text-transform: uppercase; letter-spacing: 0.08em;">Activity Fetcher</p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("**API Settings**")
    client_id     = st.text_input("Client ID",       value=DEFAULT_CLIENT_ID)
    client_secret = st.text_input(
        "Client Secret",
        type="default" if st.session_state.get("show_secret") else "password",
        value=DEFAULT_CLIENT_SECRET,
    )
    st.checkbox("👁 Show Client Secret", key="show_secret")
    org_id        = st.text_input("Organisation ID", value=DEFAULT_ORG_ID)

    st.divider()
    st.markdown("**Search**")
    mode = st.radio("Fetch Mode", ["All Students", "Specific Students"])
    student_search_first = ""
    student_search_last  = ""
    student_search_email = ""
    if mode == "Specific Students":
        student_search_first = st.text_input("First Name",     placeholder="e.g. Flynn")
        student_search_last  = st.text_input("Last Name",      placeholder="e.g. Disney")
        student_search_email = st.text_input("Email",          placeholder="e.g. ian+flynn@...")

    st.divider()
    st.markdown("**Date Range** *(required)*")
    today      = date.today()
    start_date = st.date_input("From", value=today)
    end_date   = st.date_input("To",   value=today)

    st.divider()
    st.markdown("**Options**")
    show_attendance    = st.toggle("Include Attendance Status", value=False)
    include_cancelled  = st.toggle("Include Cancelled Events", value=False)
    show_debug         = st.toggle(
        "Show Activity-Name Debug Info", value=False,
        help="Shows the raw group/CCA lookups and a few raw event records, "
             "so we can see exactly which id field actually links an event "
             "to its activity name.",
    )
    st.divider()
    fetch_btn = st.button("Fetch Activities", type="primary", use_container_width=True)
    stop_btn  = st.button("⛔  Stop", use_container_width=True)
    if stop_btn:
        st.session_state["stop"] = True

# ── Page header ───────────────────────────────────────────────
st.markdown("""
<div style="background: white; border-radius: 14px; border: 1px solid #e2daf2; padding: 1.25rem 1.75rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem;">
    <div style="background: linear-gradient(135deg, #d4489a, #7c3abf); border-radius: 12px; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0;">🎓</div>
    <div>
        <p style="font-size: 20px; font-weight: 700; color: #3b1a5e; margin: 0;">Activity Fetcher</p>
        <p style="font-size: 13px; color: #8b75b8; margin: 0;">Pull student CCA activities and parent contacts from SchoolsBuddy</p>
    </div>
</div>
""", unsafe_allow_html=True)


# ── Helpers ───────────────────────────────────────────────────
RETRY_STATUS_CODES = {429, 500, 502, 503, 504}
MAX_RETRIES = 4

def get_token(cid, csecret, scope=None):
    payload = {
        "grant_type": "client_credentials",
        "client_id": cid,
        "client_secret": csecret,
    }
    if scope:
        payload["scope"] = scope
    resp = requests.post(AUTH_URL, data=payload)
    if resp.status_code >= 400:
        # Surface the token server's own error body (e.g. IdentityServer4's
        # {"error":"invalid_client"} / {"error":"invalid_scope"}) instead of
        # just "400 Bad Request" -- that's what actually explains the failure.
        try:
            detail = resp.json()
        except ValueError:
            detail = resp.text
        raise RuntimeError(
            f"{resp.status_code} {resp.reason} from token endpoint: {detail}"
        )
    return resp.json()["access_token"]

def api_get(token, path, params=None):
    """GET with basic retry/backoff for rate limits (429) and transient 5xx errors."""
    delay = 1.5
    for attempt in range(1, MAX_RETRIES + 1):
        resp = requests.get(
            f"{BASE_URL}{path}",
            headers={"Authorization": f"Bearer {token}"},
            params=params,
            timeout=60,
        )
        if resp.status_code not in RETRY_STATUS_CODES or attempt == MAX_RETRIES:
            resp.raise_for_status()
            return resp.json()
        wait = float(resp.headers.get("Retry-After", delay))
        time.sleep(wait)
        delay *= 2

def fetch_all_pages(token, path, base_params=None):
    results, page = [], 1
    while True:
        params = {**(base_params or {}), "PageNumber": page, "PageSize": 2000}
        data   = api_get(token, path, params)
        results.extend(data.get("data") or [])
        if page >= (data.get("totalPages") or 1):
            break
        page += 1
    return results

def fetch_group_names(token, org_id):
    """Standard groups (GET /api/v1/Groups/Group) -> {groupId: name}.

    Returns (dict, error_message_or_None, raw_sample) -- callers should
    surface the error rather than silently treating a failed call the same
    as "no groups". raw_sample is up to 3 unmodified group records, for
    debugging what fields actually come back.
    """
    params = {"IncludeArchived": True}
    if org_id: params["OrganisationIds"] = org_id
    try:
        groups = fetch_all_pages(token, "/api/v1/Groups/Group", params)
        names = {
            g["id"]: g.get("name") or g.get("groupName") or g.get("shortName") or ""
            for g in groups if g.get("id") is not None
        }
        return names, None, groups[:3]
    except Exception as e:
        return {}, str(e), []

def fetch_cca_names(token, org_id):
    """CCA Sign-Up (IBP + Preference) child groups and containers.

    Events generated by a CCA sign-up don't necessarily show up under the
    standard Groups/Group endpoint -- the activity/club they belong to lives
    under CcaSignUp/{IBP|Pref}/{signupId}/groups instead. Returns
    ({groupId: name}, {signupId: containerName}, [error messages], raw_group_sample).
    raw_group_sample is up to 3 unmodified CCA group records, for debugging
    what fields actually come back.
    """
    cca_group_names  = {}
    cca_signup_names = {}
    errors = []
    raw_group_sample = []
    for container_type in ("IBP", "Pref"):
        try:
            params = {}
            if org_id: params["OrganisationIds"] = org_id
            containers = fetch_all_pages(token, f"/api/v1/CcaSignUp/{container_type}", params)
        except Exception as e:
            errors.append(f"CcaSignUp/{container_type} containers: {e}")
            continue
        for container in containers:
            signup_id = container.get("id")
            if signup_id is None:
                continue
            cca_signup_names[signup_id] = container.get("name") or ""
            try:
                org_param = {}
                if org_id: org_param["organisationId"] = org_id
                groups_resp = api_get(
                    token, f"/api/v1/CcaSignUp/{container_type}/{signup_id}/groups", org_param
                )
            except Exception as e:
                errors.append(f"CcaSignUp/{container_type}/{signup_id}/groups: {e}")
                continue
            items = groups_resp if isinstance(groups_resp, list) else (groups_resp.get("data") or [])
            for g in items:
                gid = g.get("id")
                if gid is not None:
                    # CCA group records use "groupName", not the plain
                    # "name" that standard Groups/Group records use.
                    cca_group_names[gid] = g.get("groupName") or g.get("name") or g.get("shortName") or ""
                if len(raw_group_sample) < 3:
                    raw_group_sample.append(g)
    return cca_group_names, cca_signup_names, errors, raw_group_sample

def resolve_activity_name(ev, std_group_names, cca_group_names, cca_signup_names):
    """Best-effort activity/club name for an event, trying every id field
    SchoolsBuddy events can carry before giving up. "clubId" and "groupId"
    are tried against BOTH lookups since it's unclear from the API docs
    alone which one an org's events actually populate, or whether "club"
    is just this org's word for a standard group rather than a CCA group."""
    for id_field in ("groupId", "clubId"):
        val = ev.get(id_field)
        if val is None:
            continue
        if std_group_names.get(val):
            return std_group_names[val]
        if cca_group_names.get(val):
            return cca_group_names[val]

    signup_id = ev.get("ccaSignUpId")
    if signup_id is not None and cca_signup_names.get(signup_id):
        return cca_signup_names[signup_id]

    # Nothing matched a group/club/signup lookup -- fall back to the event's
    # own name rather than a flat "(Unknown Group)" label, since that's
    # almost always more useful (and often is the activity name itself).
    return ev.get("eventName") or "(Unknown Group)"

def fetch_events(token, sc_student_id, org_id, start_date, end_date,
                  include_cancelled=False):
    params = {
        "SCStudentId":      sc_student_id,
        "PageSize":         2000,
        "IsArchived":       False,
        "IsCancelled":      include_cancelled,
        "IncludeAttendees": True,
        "StartDateTime":    f"{start_date}T00:00:00Z",
        "EndDateTime":      f"{end_date}T23:59:59Z",
    }
    if org_id: params["OrganisationIds"] = org_id
    data   = api_get(token, "/api/v1/Events", params)
    events = data.get("data") or []
    active = []
    for ev in events:
        attendees = ev.get("attendees") or []
        student_record = next(
            (a for a in attendees if str(a.get("id")) == str(sc_student_id)),
            None
        )
        if student_record:
            ev["_student_register_status"] = student_record.get("registerStatus", "Unset")
            active.append(ev)
    return active

def fetch_parent_emails(token, related_ids):
    emails = []
    for cid in (related_ids or []):
        try:
            c = api_get(token, f"/api/v1/Contacts/{cid}")
            if c.get("emailAddress"):
                emails.append(c["emailAddress"])
        except Exception:
            pass
    return emails

WEEKDAYS  = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]
TZ_UTC    = timezone(timedelta(0))
TZ_PLUS8  = timezone(timedelta(hours=8))

def to_plus8(dt_str):
    if not dt_str: return None
    try:
        clean  = dt_str.replace("Z","").split("+")[0]
        dt_utc = datetime.fromisoformat(clean).replace(tzinfo=TZ_UTC)
        return dt_utc.astimezone(TZ_PLUS8)
    except Exception:
        return None

def fmt_time(dt_str):
    dt = to_plus8(dt_str)
    return dt.strftime("%H:%M") if dt else ""

def fmt_weekday(dt_str):
    dt = to_plus8(dt_str)
    return WEEKDAYS[dt.weekday()] if dt else ""


def build_xlsx(export_df, start_date, end_date):
    """Build a formatted .xlsx workbook (bytes) from the export dataframe."""
    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        sheet_name = "Student Activity"
        export_df.to_excel(writer, index=False, sheet_name=sheet_name)
        ws = writer.sheets[sheet_name]

        header_fill = PatternFill(start_color="3B1A5E", end_color="3B1A5E", fill_type="solid")
        header_font = Font(color="FFFFFF", bold=True)
        clash_fill  = PatternFill(start_color="FFE0E0", end_color="FFE0E0", fill_type="solid")

        for col_idx, col_name in enumerate(export_df.columns, start=1):
            cell = ws.cell(row=1, column=col_idx)
            cell.fill = header_fill
            cell.font = header_font
            cell.alignment = Alignment(vertical="center")

        clash_col_idx = (
            list(export_df.columns).index("Clash") + 1 if "Clash" in export_df.columns else None
        )
        if clash_col_idx:
            for row_idx in range(2, ws.max_row + 1):
                if ws.cell(row=row_idx, column=clash_col_idx).value == "CLASH":
                    for col_idx in range(1, ws.max_column + 1):
                        ws.cell(row=row_idx, column=col_idx).fill = clash_fill

        for col_idx, col_name in enumerate(export_df.columns, start=1):
            # Don't rely on .astype(str) alone having made every value a real
            # str (NaN/None/numpy scalars can slip through depending on
            # dtype) -- wrap explicitly in str() so len() never blows up.
            sample = export_df.iloc[:2000, col_idx - 1]
            max_len = max([len(str(col_name))] + [len(str(v)) for v in sample])
            ws.column_dimensions[get_column_letter(col_idx)].width = min(max(max_len + 2, 10), 50)

        ws.freeze_panes = "A2"
        ws.auto_filter.ref = ws.dimensions

    buf.seek(0)
    return buf.getvalue()


# ── Fetch logic ───────────────────────────────────────────────
if fetch_btn:
    st.session_state["stop"]    = False
    st.session_state["fetched"] = False

    if not client_id or not client_secret:
        st.error("Please enter your Client ID and Client Secret.")
        st.stop()

    if start_date > end_date:
        st.error("'From' date must be before 'To' date.")
        st.stop()

    with st.spinner("Authenticating..."):
        try:
            token = get_token(client_id, client_secret)
        except Exception as e:
            st.error(f"Authentication failed: {e}")
            st.stop()

    with st.spinner("Loading activity names..."):
        std_group_names, group_err, std_group_raw_sample = fetch_group_names(token, org_id)
        cca_group_names, cca_signup_names, cca_errs, cca_group_raw_sample = fetch_cca_names(token, org_id)

        lookup_errors = ([group_err] if group_err else []) + cca_errs
        if lookup_errors:
            st.warning(
                "Some activity-name lookups failed, so a few events may show "
                "their raw event name instead of a group/club name:\n\n"
                + "\n".join(f"- {e}" for e in lookup_errors)
            )
        elif not std_group_names and not cca_group_names:
            st.info(
                "No standard groups or CCA sign-up groups were found for this "
                "organisation — activity names will fall back to each event's own name."
            )

    with st.spinner("Fetching students..."):
        try:
            if mode == "Specific Students":
                first_q = student_search_first.strip()
                last_q  = student_search_last.strip()
                email_q = student_search_email.strip()

                if not any([first_q, last_q, email_q]):
                    st.error("Please enter at least one search field.")
                    st.stop()

                # Search API separately for each field, then intersect
                def search_students(query):
                    params = {"Search": query, "PageSize": 2000}
                    if org_id: params["OrganisationIds"] = org_id
                    data = api_get(token, "/api/v1/Students", params)
                    return {s["id"]: s for s in (data.get("data") or [])}

                result_sets = []
                if first_q:
                    result_sets.append(search_students(first_q))
                if last_q:
                    result_sets.append(search_students(last_q))
                if email_q:
                    result_sets.append(search_students(email_q))

                # Intersect by student ID
                if len(result_sets) == 1:
                    combined = result_sets[0]
                else:
                    common_ids = set(result_sets[0].keys())
                    for rs in result_sets[1:]:
                        common_ids &= set(rs.keys())
                    combined = {sid: result_sets[0][sid] for sid in common_ids if sid in result_sets[0]}

                students = list(combined.values())

                if not students:
                    st.warning("No students found matching your search.")
                    st.stop()
            else:
                params = {}
                if org_id: params["OrganisationIds"] = org_id
                students = fetch_all_pages(token, "/api/v1/Students", params)
        except Exception as e:
            st.error(f"Failed to fetch students: {e}")
            st.stop()

    st.info(f"Found **{len(students)}** student(s) — fetching activities and parent info...")

    rows        = []
    progress    = st.progress(0)
    status_text = st.empty()
    raw_event_samples = []

    for i, student in enumerate(students):
        sc_id    = student.get("id")
        first    = student.get("firstName", "")
        last     = student.get("lastName", "")
        grade    = student.get("grade", "")
        homeroom = student.get("homeroomClass", "")
        email    = student.get("email", "")
        related  = student.get("relatedPeople") or []

        status_text.text(f"Processing {i+1}/{len(students)}: {first} {last}")

        if st.session_state.get("stop"):
            status_text.warning(f"⛔ Stopped after processing {i} student(s).")
            break

        try:
            events = fetch_events(
                token, sc_id, org_id, start_date, end_date,
                include_cancelled=include_cancelled,
            )
        except Exception:
            events = []

        # Students with nothing in this date range don't need a row at all.
        if events:
            try:
                parent_emails = fetch_parent_emails(token, related)
            except Exception:
                parent_emails = []

            parent_str = "; ".join(parent_emails)

            for ev in events:
                day_str   = fmt_weekday(ev.get("eventStartTime"))
                day_order = WEEKDAYS.index(day_str) if day_str in WEEKDAYS else 99
                start_dt  = to_plus8(ev.get("eventStartTime"))
                end_dt    = to_plus8(ev.get("eventEndTime"))

                if show_debug and len(raw_event_samples) < 8:
                    # Dump the FULL raw event record (minus the bulky attendee
                    # list) rather than a curated subset of fields -- if the
                    # activity name lives on some undocumented field we
                    # haven't thought to look for, this is how we'd spot it.
                    ev_copy = {k: v for k, v in ev.items() if k != "attendees"}
                    ev_copy["_attendee_count"]  = len(ev.get("attendees") or [])
                    ev_copy["_student"]         = f"{first} {last}"
                    ev_copy["_resolved_name"]   = resolve_activity_name(
                        ev, std_group_names, cca_group_names, cca_signup_names
                    )
                    raw_event_samples.append(ev_copy)

                rows.append({
                    "First Name":      first,
                    "Last Name":       last,
                    "Student Email":   email,
                    "Grade":           grade,
                    "Homeroom":        homeroom,
                    "Parent Email(s)": parent_str,
                    "Activity Name":   resolve_activity_name(
                        ev, std_group_names, cca_group_names, cca_signup_names
                    ),
                    "Event Type":      ev.get("eventName", ""),
                    "Day":             day_str,
                    "Day Order":       day_order,
                    "Start Time":      fmt_time(ev.get("eventStartTime")),
                    "End Time":        fmt_time(ev.get("eventEndTime")),
                    "_start_dt":       start_dt,
                    "_end_dt":         end_dt,
                    "Location":        ev.get("location") or ev.get("meetingLocation") or "",
                    "Teacher":         ev.get("leadStaffMember", ""),
                    "Attendance":      ev.get("_student_register_status", "Unset"),
                })

        progress.progress((i + 1) / len(students))

    status_text.empty()
    progress.empty()

    df = pd.DataFrame(rows)
    st.session_state["df"]      = df
    st.session_state["fetched"] = True

    if show_debug:
        st.session_state["debug_info"] = {
            "std_group_count":     len(std_group_names),
            "cca_group_count":     len(cca_group_names),
            "cca_signup_count":    len(cca_signup_names),
            "std_group_sample":    dict(list(std_group_names.items())[:10]),
            "cca_group_sample":    dict(list(cca_group_names.items())[:10]),
            "cca_signup_sample":   dict(list(cca_signup_names.items())[:10]),
            "std_group_raw":       std_group_raw_sample,
            "cca_group_raw":       cca_group_raw_sample,
            "raw_event_samples":   raw_event_samples,
        }
    else:
        st.session_state.pop("debug_info", None)


# ── Results ───────────────────────────────────────────────────
if st.session_state.get("fetched") and "df" in st.session_state:
    df = st.session_state["df"]

    total_students = df["Student Email"].nunique()
    total_events   = len(df[df["Start Time"] != ""])

    col1, col2 = st.columns(2)
    col1.metric("Students", total_students)
    col2.metric("Activities Found", total_events)

    st.divider()

    col_a, col_b, col_c, col_d = st.columns(4)
    with col_a:
        search_first  = st.text_input("🔎 Student First Name", placeholder="e.g. Flynn")
    with col_b:
        search_last   = st.text_input("🔎 Student Last Name",  placeholder="e.g. Disney")
    with col_c:
        search_semail = st.text_input("✉️ Student Email", placeholder="e.g. ian+flynn@...")
    with col_d:
        search_pemail = st.text_input("✉️ Parent Email", placeholder="e.g. parent@...")

    filtered = df.copy()
    if search_first:
        filtered = filtered[filtered["First Name"].str.contains(search_first, case=False, na=False)]
    if search_last:
        filtered = filtered[filtered["Last Name"].str.contains(search_last, case=False, na=False)]
    if search_semail:
        filtered = filtered[filtered["Student Email"].str.contains(search_semail, case=False, na=False)]
    if search_pemail:
        filtered = filtered[filtered["Parent Email(s)"].str.contains(search_pemail, case=False, na=False)]

    filtered = filtered.sort_values("Day Order").drop(columns=["Day Order"])

    # ── Clash detection ───────────────────────────────────────
    def find_clashes(df):
        clash_indices = set()
        students = df["Student Email"].unique()
        for student in students:
            student_rows = df[df["Student Email"] == student].copy()
            idxs = student_rows.index.tolist()
            for i in range(len(idxs)):
                for j in range(i + 1, len(idxs)):
                    a = student_rows.loc[idxs[i]]
                    b = student_rows.loc[idxs[j]]
                    a_start, a_end = a.get("_start_dt"), a.get("_end_dt")
                    b_start, b_end = b.get("_start_dt"), b.get("_end_dt")
                    if None in (a_start, a_end, b_start, b_end):
                        continue
                    # Check overlap: two intervals overlap if start of one < end of other
                    if a_start < b_end and b_start < a_end:
                        clash_indices.add(idxs[i])
                        clash_indices.add(idxs[j])
        return clash_indices

    clash_indices = find_clashes(filtered)
    total_clashes = len(set(
        filtered.loc[i, "Student Email"]
        for i in clash_indices
        if i in filtered.index
    ))

    # Show clash warning
    if clash_indices:
        st.warning(f"⚠️ **{total_clashes} student(s)** have scheduling clashes — highlighted in red below.")

    # Display columns only (hide internal _start_dt, _end_dt)
    hidden = ["_start_dt", "_end_dt"]
    if not show_attendance:
        hidden.append("Attendance")
    display_cols = [c for c in filtered.columns if c not in hidden]
    display_df   = filtered[display_cols]

    def highlight_clashes(row):
        if row.name in clash_indices:
            return ["background-color: #ffe0e0; color: #c0392b;"] * len(row)
        return [""] * len(row)

    styled = display_df.style.apply(highlight_clashes, axis=1)
    st.dataframe(styled, use_container_width=True, hide_index=True, height=500)

    st.divider()

    drop_cols = ["Day Order", "_start_dt", "_end_dt"]
    if not show_attendance:
        drop_cols.append("Attendance")
    export_df = df.drop(columns=drop_cols, errors="ignore")
    # Add Clash column to export
    clash_indices_full = find_clashes(df)
    export_df["Clash"] = export_df.index.map(lambda i: "CLASH" if i in clash_indices_full else "")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_bytes  = export_df.to_csv(index=False, encoding="utf-8-sig")
    xlsx_bytes = build_xlsx(export_df, start_date, end_date)

    exp_col1, exp_col2 = st.columns(2)
    with exp_col1:
        st.download_button(
            label="⬇️ Export to CSV",
            data=csv_bytes,
            file_name=f"sb_activities_{timestamp}.csv",
            mime="text/csv",
            use_container_width=True,
        )
    with exp_col2:
        st.download_button(
            label="⬇️ Export to Excel (.xlsx)",
            data=xlsx_bytes,
            file_name=f"sb_activities_{timestamp}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
    st.caption("Activity data is fetched live from SchoolsBuddy.")

    if "debug_info" in st.session_state:
        dbg = st.session_state["debug_info"]
        st.divider()
        with st.expander("🐛 Activity-name debug info", expanded=True):
            st.markdown(
                f"**Lookups loaded:** {dbg['std_group_count']} standard group(s), "
                f"{dbg['cca_group_count']} CCA group(s) across "
                f"{dbg['cca_signup_count']} CCA sign-up container(s)."
            )
            st.markdown("**Sample of loaded group names (id → name):**")
            st.json({
                "standard_groups": dbg["std_group_sample"],
                "cca_groups": dbg["cca_group_sample"],
                "cca_signups": dbg["cca_signup_sample"],
            })
            st.markdown(
                "**Raw group/CCA-group records** (unmodified API response — "
                "check this if names look blank or wrong even when an id "
                "*does* match, since that means the wrong field is being "
                "read for the name itself):"
            )
            st.json({
                "standard_groups_raw": dbg["std_group_raw"],
                "cca_groups_raw": dbg["cca_group_raw"],
            })
            st.markdown(
                "**Sample of raw events** (the id fields here should "
                "match a key in one of the dicts above — whichever one "
                "doesn't match anything is why the name isn't resolving; "
                "`_resolved_name` shows what the app currently picked):"
            )
            st.json(dbg["raw_event_samples"])
